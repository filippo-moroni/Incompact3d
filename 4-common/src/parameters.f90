...
subroutine parameter(input_i3d)
  ...
  if (itype .eq. itype_channel) then
      ...
      if (cpg) then
          ...
          ! Calculate Reynolds centerline of the related laminar Poiseuille flow (re = Re_tau in this case)
  ...
  if (iimplicit.ne.0) then
     ...
     if (isecondder==5) then     
        ...
        if (nrank==0) write(*,*) "          BCs on both boundaries (ncly1 = nclyn = 2).      "     
     endif

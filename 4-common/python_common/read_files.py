...
def read_data(itype, numscalar, post_mean, post_grad, post_corz, post_tke_eq, ny, nz, nu, snap_numb=None):
    ...
    if itype == 3:
        ...
        # Read of TKE 
        if post_tke_eq:
            M = np.loadtxt('data_post/tke_stats.txt', skiprows=38, delimiter=',', dtype=np.float64)
            tke_turbt  = M[:,0]   
            tke_presst = M[:,1]     
            tke_difft  = M[:,2]
            tke_prod   = M[:,3]
    ...
    # TTBL
        ...
        if i_switch_plot == False:
            ...
            # Read of TKE
            if post_tke_eq: 
                M = np.loadtxt(f'data_post/tke_stats-{snap_numb}.txt', skiprows=40, delimiter=',', dtype=np.float64)
                tke_turbt  = M[:,0]   
                tke_presst = M[:,1]     
                tke_difft  = M[:,2]
                tke_prod   = M[:,3]
...
def read_ref_data():
    ...
    # Lee & Moser (2015), Channel Re_tau = 1000, ratio of production and dissipation of TKE - 1.0 
